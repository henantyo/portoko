...
fd.append('projectId', editingProject?.id || projectForm?.id || '');
              ...
              {activeTab === 'supabase' && (
                <div className="space-y-6 text-left">
                  ...
                  <form onSubmit={handleSaveSupabaseConfig} className="space-y-4 font-mono text-xs border border-[var(--border-main)] bg-[var(--bg-surface)] p-5">
                    <div className="text-[var(--accent-cyan)] font-bold mb-2 flex items-center gap-1.5">
                    ...
                    <div className="space-y-3">
                      <div className="space-y-1">
                        <label className="text-[var(--text-muted)] text-[10px] uppercase">SUPABASE_URL</label>
                        <input
                          type="url"
                          required
                      ...
                      <div className="space-y-1">
                        <label className="text-[var(--text-muted)] text-[10px] uppercase">SUPABASE_ANON_KEY (PUBLIC API KEY)</label>
                        <input
                          type="password"
                          required
                    ...
                    <div className="flex justify-between items-center pt-2">
                      <span className="text-[10px] text-[var(--text-muted)]">
                        Status: {isSupabaseConfigured() ? (
                          <span className="text-emerald-400 font-bold">CONNECTED TO CLOUD</span>
                        ) : (
                          <span className="text-yellow-500 font-bold">LOCAL OFFLINE FALLBACK</span>
                        )}
                      </span>
                      <GlowButton type="submit" variant="cyan" className="text-[10px] py-1.5 px-4">
                        <Check className="w-4 h-4" />
                  ...
                  {isSupabaseConfigured() && (
                    <div className="border border-[var(--border-main)] bg-[var(--bg-surface)] p-5 space-y-4">
                      <div className="text-[var(--accent-cyan)] font-bold flex items-center gap-1.5">
                        <Database className="w-4 h-4" />
                        <span>2. SYNCHRONIZE DATA PACKETS</span>
                      </div>
                      <p className="font-mono text-[10px] text-[var(--text-tertiary)] leading-relaxed">
                        Gunakan tombol di bawah ini untuk memindahkan data dari Penyimpanan Lokal browser Anda ke Supabase Cloud (Push), atau sebaliknya (Pull).
                      </p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                        ...
                        >
                          ...
                          <span>{isPushing ? 'PUSHING...' : 'MIGRATE LOCAL DATA TO CLOUD (PUSH)'}</span>
                        </GlowButton>
                        ...
                        >
                          ...
                          <span>{isPulling ? 'PULLING...' : 'FETCH CLOUD DATA TO LOCAL (PULL)'}</span>
                        </GlowButton>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="border border-dashed border-cyan-500/15 bg-[var(--bg-surface-strong)] p-5 space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-[var(--accent-pink)] font-bold uppercase tracking-wider">// 3. SQL SCHEMA SETUP INSTRUCTIONS</span>
                      <GlowButton onClick={handleCopySQL} variant="neutral" className="text-[9px] py-1 px-2.5 flex items-center gap-1">
                    ...
                    <p className="font-mono text-[10px] text-[var(--text-tertiary)] leading-relaxed">
                      Sebelum menghubungkan Supabase, masuklah ke **Supabase Dashboard &gt; SQL Editor** di akun Anda, lalu paste dan jalankan (Run) script SQL di bawah ini untuk membuat tabel-tabel secara otomatis:
                    </p>

                    <pre className="w-full h-40 overflow-y-auto bg-[var(--bg-surface-strong)] text-[var(--text-muted)] p-4 border border-cyan-500/5 font-mono text-[9px] leading-relaxed text-left">
                      {SUPABASE_SQL_SETUP}
                    </pre>
                  </div>
                </div>
              ...
              {activeTab === 'security' && (
                <div className="space-y-6 text-left">
                  <div className="border-b border-[var(--border-main)] pb-4 mb-4">
                    <h3 className="font-rajdhani text-lg font-bold text-[var(--text-primary)] tracking-wider flex items-center gap-2">
                      <Lock className="w-5 h-5 text-[var(--accent-pink)]" />
                      <span>SECURITY & AUTHENTICATION</span>
                    </h3>
                    <p className="font-mono text-[10px] text-[var(--text-muted)]">Manage admin password and session security parameters.</p>
                  </div>

                  {/* Change Password Form */}
                  <form onSubmit={handleChangePassword} className="space-y-4 font-mono text-xs border border-pink-500/10 bg-[var(--bg-surface-strong)] p-6">
                    <div className="flex items-center gap-2 border-b border-pink-500/10 pb-3 mb-4">
                      <Lock className="w-4 h-4 text-[var(--accent-pink)]" />
                      <span className="font-bold text-[var(--accent-pink)] uppercase tracking-wider">CHANGE ADMIN PASSWORD</span>
                    </div>

                    <div className="space-y-1">
                      <label className="text-[var(--text-muted)] text-[10px] uppercase">Current Password</label>
                      <input
                        type="password"
                        required
                        ...
                        className="w-full bg-[var(--bg-input)] border border-pink-500/10 p-2 text-[var(--text-primary)] focus:outline-none focus:border-pink-400"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-[var(--text-muted)] text-[10px] uppercase">New Password</label>
                      <input
                        type="password"
                        required
                        value={newPassword}
                    ...
                    <div className="space-y-1">
                      <label className="text-[var(--text-muted)] text-[10px] uppercase">Confirm New Password</label>
                      <input
                        type="password"
                        required
                    ...
                    <div className="pt-4 flex justify-end">
                      ...
                      >
                        ...
                        <span>{isChangingPassword ? 'UPDATING...' : 'UPDATE_PASSWORD'}</span>
                      </GlowButton>
                    </div>
                  </form>

                  {/* Security Tips */}
                  <div className="border border-dashed border-pink-500/15 bg-[var(--bg-surface-strong)] p-5 space-y-3">
                    <span className="text-[var(--accent-pink)] font-bold uppercase tracking-wider block">// SECURITY BEST PRACTICES</span>
                    <ul className="space-y-2 text-[10px] text-[var(--text-tertiary)] list-disc list-inside">
                      <li>✓ Gunakan password yang kuat dengan kombinasi huruf, angka, dan simbol</li>
                      <li>✓ Jangan bagikan password admin Anda kepada siapa pun</li>
                      <li>✓ Logout setelah selesai mengelola dashboard</li>
                      <li>✓ Jangan simpan password di tempat umum atau tidak aman</li>
                      <li>✓ Password di-hash menggunakan bcrypt untuk keamanan maksimal</li>
                    </ul>
                  </div>

                  {/* Session Info */}
                  <div className="border border-[var(--border-main)] bg-[var(--bg-surface-strong)] p-4 font-mono text-[10px] space-y-2 text-[var(--text-muted)]">
                    <div className="font-bold text-[var(--accent-cyan)] mb-3">CURRENT SESSION INFORMATION</div>
                    <div>Status: <span className="text-emerald-400 font-semibold">AUTHENTICATED</span></div>
                    <div>Auth Token: <span className="text-slate-600 text-[9px]">[Token stored securely in localStorage]</span></div>
                    <div>Session Type: <span className="text-yellow-500 font-semibold">LOCAL_SESSION</span></div>
                  </div>
                </div>
              )}
            </div>
